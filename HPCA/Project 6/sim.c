...
int cache_upgrades[NUM_PROCESSORS], cache_writebacks[NUM_PROCESSORS], silent_upgrades[NUM_PROCESSORS];
int converted_cache_upgrades[NUM_PROCESSORS];
int numCacheToCacheTransfer[NUM_PROCESSORS], numMemToCacheTransfer[NUM_PROCESSORS];
...
int PFlag, PBit[NUM_PROCESSORS];  // Presence Flag
int OFlag, OBit[NUM_PROCESSORS]; // Owner flag
int CACHE_READY;
int CACHE_TRANSFER;
...
void cleanUp()  {
  ...
  for (i=0; i < NUM_PROCESSORS; i++) {
    printf("\nProcessor: %d\tREAD HITS: %d\tREAD MISSES: %d\tWRITE HITS: %d\tUPGRADES: %d\t SILENT UPGRADES: %d\tCONVERTED UPGRADES: %d\tWRITE MISSES: %d\tWRITEBACKS: %d\tTime: %5.2f\n", i, cache_read_hits[i], cache_read_misses[i], cache_write_hits[i],   cache_upgrades[i], silent_upgrades[i], converted_cache_upgrades[i], cache_write_misses[i],cache_writebacks[i],GetSimTime());
