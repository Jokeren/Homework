...
extern diplayCacheBlock(int, int);
  ...
  if ((CACHE[procId][blkNum].TAG != myTag)|| (CACHE[procId][blkNum].STATE == I)) { // Cache MISS
    ...
    switch (op) {
      ...
      case STORE:
        ...
        if (TRACE)
          printf("procId %d\tWrite Misses: %d Address: %x\n", procId, cache_write_misses[procId], req->address);
        MakeBusRequest(procId, req);  // Reads block into cache
...
/* ******************************************************************************************
   ...
   On cache hit: A silent transaction is applied directly to the cache; returns after delay of 1  CLOCK_CYCLE.
...
int LookupCache(int procId, struct genericQueueEntry *req) {
  ...
  if ((CACHE[procId][blkNum].TAG == myTag) && (CACHE[procId][blkNum].STATE != I)) {   // Cache Hit
    if (TRACE)
      printf("Processor %d --  Cache Hit: Address %x Time %5.2f\n", procId,req->address,GetSimTime());
    ...
    switch (CACHE[procId][blkNum].STATE) {
      case M: { // Silent Transaction
                switch(op) {
                  ...
                  case STORE: 
                    ...
                    if (TRACE)
                      printf("procId %d\tWrite Hits: %d Address: %x\n", procId, cache_write_hits[procId], req->address);
      ...
      case S: { 
                switch(op) {
                  ...
                  case STORE:
                    if (TRACE)
                      printf("Processor  %d: Need UPGRADE -- Address %x Time %5.2f\n", procId,req->address,GetSimTime());
                    ...
                    // If my UPGRADE request was not interruped by another processor's write (INV or BUS_RDX) my cache block state is still SM; 
                    // else it is I
