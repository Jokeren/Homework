...
void FlushDirtyBlock(struct genericQueueEntry * writeBlk) {
  ...
  /*
     1. Wait (if necessary) till there is space in the Writeback queue for a new request,

     2. Add the write  request to the Writeback queue using the appropriate function from "utils.o"

     3. Signal the writeback thread of the availability of an additional request in the Writeback queue.
...
*/
  SemaphoreWait(sem_writeQueueFull);  // --1
  SemaphoreWait(sem_writebackQueue); 
...
int * MemoryRead(struct genericQueueEntry *memreq) {
  ...
  /*  
      Delays the caller by either WRITE_BUFFER_SEARCH_DELAY  or an additional  MEMORY_READ_TIME depending on whether the request is found in the Writeback Queue or requires to be fetched from memory.

      1. Check if a write to the address matching that of the request is pending in the Writeback Queue. Use  a call to an appropriate function in "util.o".  The time to search the  Writeback Queue is  WRITE_BUFFER_SEARCH_DELAY and the process should delay by this amount to simulate the time spent in the search. 

      2. If request is not not found  in the Writeback Queue, we need to do a memory operation. Simulate the memory read by an additional delay of MEMORY_READ_TIME and return.

      3. The function returns a NULL pointer when done as shown.

NOTES:
1. Make sure you have exclusive access to the Writeback Queue in Step 1.


2. Make sure you have exclusive access to the memory using "sem_memory" in Step 2 before delaying. 

There are statements for gathering statistics that are present below along with debug printf statements. Move them into your code at appropriate points.
*/
