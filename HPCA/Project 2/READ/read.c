...
int printStatistics(int normalCompletion) {
  ...
  printf("Total number Read Misses Found In WriteQ: %d\n",   numReadsFoundInWriteQ);  
...
void memorywriter(){
  ...
  if (TRACE)
    printf("\nMemory Write Thread starts  at time %5.2f\n", GetSimTime());
  ...
  while(1){
    ...
    if (TRACE)
      printf("Memory WRITER: Contend for memory access semaphore at time %5.2f\n", GetSimTime());
...
int * MemoryRead(struct genericQueueEntry *memreq) {
  ...
  if (TRACE)
    printf("MemoryRead: Search Writeback Queue for address %x Time %5.2f\n", memreq->reqAddress, GetSimTime());
  ...
  if (TRACE)
    printf("Memory Read: Got memory access semaphore to read block %x. Time %5.2f\n", (memreq->reqAddress)/BLKSIZE, GetSimTime());
...
int ServiceMemRequest() {
  ...
  if (TRACE)
    printf("\nIn ServiceMemoryRequest: Thread: %d Address: %x Block: %x Type: %s Time: %5.2f\n", req->threadId, req->reqAddress, (req->reqAddress / BLKSIZE), (req->type==0) ? "WRITE": "READ", GetSimTime());
...
int LookupCache(unsigned address, unsigned type) {
  ...
  for (way=0; way < NUMWAYS; way++)     {
    if ( (CACHE[set_index][way].V == TRUE) && (CACHE[set_index][way].TAG == my_tag)) {
      ...
      if (type == WRITE) { 
        if (TRACE)
          printf("Setting lookup block to DIRTY. Set Index: %d way: %d block_num : %x\n", set_index, way, block_num);
...
void processor()
{
  ...
  while(1){
    ...
    if (TRACE)
      printf("Thread %d makes request for Memory Address %x Block Number: %x at time %5.2f\n", thread_num, mem_ref->address, mem_ref->address/BLKSIZE, GetSimTime());
...
initialize() {
  ...
  for (i=0; i < MAX_NUM_THREADS; i++) {
    ...
    Misses[i] = 0;
    numrecords[i]= 0;
    totalTimeCPUBusy[i] = 0.0;
    lastTimeCPUBusy[i] = 0.0;
  }
  totalHits = 0;
  totalMisses = 0;
  totalFlushes = 0;
  totalWritebacksCompleted = 0;
  numReadsFoundInWriteQ = 0;

  totalMemoryIdleTime = 0.0;
  totalMemoryReadTime = 0.0;
  totalMemoryWriteTime = 0.0;
  lastMemoryIdleTime = 0.0;
